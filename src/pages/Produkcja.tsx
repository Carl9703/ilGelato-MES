                          ...
                          {s.sugerowane_partie && s.sugerowane_partie.length > 0 ? (
                            s.sugerowane_partie.map(p => (
                              <div key={p.id} className="flex justify-between items-center bg-[var(--bg-panel)] py-2 px-3 rounded-lg border border-[var(--border)] text-sm">
                                ...
                                <span className="text-[var(--text-muted)] text-xs">Stan: {fmtL(p.stan, 3)}</span>
                              </div>
                            ))
                          ) : (
                        ...
                        <div className="space-y-1">
                          {wizWyroby.map(w => {
                            ...
                            return (
                              <div key={w._key} className="flex justify-between items-center gap-2">
                                <span className="truncate" style={{ color: 'var(--text-secondary)' }}>{rec?.asortyment_docelowy.nazwa}</span>
                                <span className={`font-mono font-bold shrink-0 ${kg > 0 ? 'text-emerald-400' : 'text-slate-500'}`}>{fmtL(kg, 3)} kg</span>
                              </div>
                            );
                          })}
                        </div>
                                            ...
                                            {w.planowane_opakowania.map((op, idx) => (
                                              <div key={idx} className="flex items-center gap-1.5">
                                                ...
                                                <button onClick={() => setWizWyroby(prev => prev.map(x => x._key === w._key ? { ...x, planowane_opakowania: x.planowane_opakowania?.filter((_, i) => i !== idx) } : x))}
                                                  className="text-slate-600 hover:text-red-400 transition-colors ml-2"><Trash2 className="w-3.5 h-3.5" /></button>
                                              </div>
                                            ))}
                                          </div>
                                        )}
                              ...
                              <div key={w.id} className="bg-[var(--bg-surface)] rounded-xl border border-[var(--border)] p-4">
                                ...
                                <div className="grid grid-cols-3 gap-2 text-center mb-3">
                                  <div>
                                    ...
                                    <div className="mono font-bold text-[var(--text-primary)] text-sm">{fmtL(w.planowana_ilosc_wyrobu, 3)}</div>
                                  </div>
                                  <div>
                                ...
                                {opWyrob.length > 0 && (
                                  <div className="text-[11px] text-[var(--text-muted)] border-t border-[var(--border)] pt-2 mt-1">
                                    ...
                                    {opWyrob.length} szt. · {fmtL(opWagaTot, 3)} kg
                                  </div>
                                )}
                              </div>
                            );
                                  ...
                                  {opSesji.map((op, i) => (
                                    <div key={i} className="flex items-center justify-between px-4 py-2.5 text-sm hover:bg-[var(--bg-surface)]/40">
                                      ...
                                      <div className="text-right">
                                        ...
                                        <span className="text-[10px] text-[var(--text-muted)] ml-1">= {fmtL(op.count * op.waga_kg, 3)} kg</span>
                                      </div>
                                    </div>
                                  ))}
                                </div>
